...
def send_detection_request(image_path, classes):
    ...
    with open(image_path, 'rb') as image_file:
        image_base64 = base64.b64encode(image_file.read()).decode(
            'utf-8')  # read后是二进制格式，encode后是Base64编码仍然是二进制，decode为utf-8
    ...
    # 发送 POST 请求，看GDINO是部署在本地还是服务器上
    response = requests.post('http://127.0.0.1:5000/detect', json=data)# 本地回环
    # response = requests.post('http://10.104.x.xx:5000/detect', json=data) #服务器
    # 处理响应
...
if __name__ == '__main__':
    ...
    classes = 'big bowl'  # 替换为你需要的类别,可以添加
