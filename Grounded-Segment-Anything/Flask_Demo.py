...
def draw_detections(image_path, detections):
    ...
    for detection in detections:
        print(detection,'----------')
        x1, y1, x2, y2, score, label  = detection
        # 转换坐标为像素值
        print(x1, y1, x2, y2)
...
def send_detection_request(image_path, classes):
    ...
    with open(image_path, 'rb') as image_file:
        image_base64 = base64.b64encode(image_file.read()).decode('utf-8') #read后是二进制格式，encode后是Base64编码仍然是二进制，decode为utf-8
...
if __name__ == '__main__':
    image_path = 'assets/demo2.jpg'  # 替换为你的图片路径
    classes = 'dog'  # 替换为你需要的类别,可以添加
